...
import { addDays, formatDay, formatTime, monthStart, weekStart } from "@/lib/dates";
...
import type { Attendance, Member, Plan, Session, Subscription } from "@/lib/types";
...
function DoorBox({
    ...
    return (
      <div className="door">
        <p className="door-title">
          {first} has used her {allowance} {allowance === 1 ? "session" : "sessions"} this week.
        </p>
        ...
        <form action={cashExtra}>
          <Hidden name="attendanceId" value={a.id} />
          <input name="amount" inputMode="decimal" placeholder="£ cash received" aria-label="Cash received in pounds" required />
        ...
        <form action={undoHere}>
          <Hidden name="attendanceId" value={a.id} />
          <button type="submit" className="btn btn-outline-brick btn-block">
            Not today (undo)
          </button>
        </form>
      </div>
  ...
  return (
    <div className="door">
      <p className="door-title">{first} has no plan this month. Record cash for:</p>
      {plans.map((p) => (
...
export default async function RegisterPage({
  ...
  const { data: sessionRow } = await supabase.from("sessions").select("*").eq("id", id).maybeSingle();
  if (!sessionRow) notFound();
  const session = sessionRow as Session;

  const ws = weekStart(session.session_date);
  const [{ data: membersData }, { data: subsData }, { data: weekSessions }, { data: plansData }] = await Promise.all([
  ...
  const subByMember = new Map(((subsData ?? []) as Subscription[]).map((s) => [s.member_id, s]));
  ...
  const hereCount = members.filter((m) => (attByMember.get(m.id) ?? []).some((a) => a.session_id === id)).length;
  const undecided = members.filter((m) =>
    (attByMember.get(m.id) ?? []).some((a) => a.session_id === id && a.flag && !a.resolution),
  ).length;
  ...
  return (
    <>
      ...
      <PageHead title="Register" sub={`${formatDay(session.session_date)}, ${formatTime(session.start_time)}`}>
        <div className="cluster" style={{ marginTop: 12 }}>
          <span className="chip chip-solid">{hereCount} here</span>
          {undecided > 0 ? <span className="chip chip-warn">{undecided} to decide</span> : null}
          {session.cancelled ? <span className="chip chip-warn">Cancelled</span> : null}
        </div>
      </PageHead>

      <form method="get" role="search" style={{ marginBottom: 14 }}>
        <input name="q" type="search" placeholder="Search name" defaultValue={q ?? ""} aria-label="Search by name" />
      </form>
              ...
              <li key={m.id}>
                <div className="row-main">
                  ...
                  <div className="grow">
                    ...
                    <div className={line.tone === "ok" ? "sub" : `sub ${line.tone}`}>{line.text}</div>
                  </div>
                  <form action={toggleHere}>
                    ...
                    <button
                      ...
                      aria-pressed={!!here}
                      aria-label={here ? `${m.name} is here. Tap to undo.` : `Mark ${m.name} as here`}
                    >
                ...
                {needsDecision && here ? (
                  <DoorBox a={here} member={m} allowance={sub?.sessions_per_week ?? 0} plans={plans} />
                ) : null}
              </li>
            );
          })}
          {shown.length === 0 ? <li className="empty">No one called &ldquo;{q}&rdquo;.</li> : null}
        </ul>
      )}
      ...
      <div className="stack" style={{ marginTop: 20 }}>
        <details className="details">
          ...
          <form action={addWalkIn}>
            ...
            <div className="field">
              ...
              <input id="walkin-phone" name="phone" type="tel" inputMode="tel" />
            </div>
            <button type="submit" className="btn btn-primary btn-block">
              Add and mark here
            </button>
        ...
        <form action={setCancelled}>
          <Hidden name="sessionId" value={id} />
          <Hidden name="cancelled" value={session.cancelled ? "false" : "true"} />
          <button type="submit" className="btn btn-quiet btn-block">
            {session.cancelled ? "Bring this session back" : "Cancel this session"}
          </button>
        </form>
      </div>
    </>
