...
import { addDays, addMonths, formatDate, monthName, monthStart, todayISO } from "@/lib/dates";
...
export default async function MembersPage({
  ...
  const filter = (["all", "notpaid", "notseen", "owes"].includes(params.filter ?? "") ? params.filter : "all") as Filter;
  ...
  const [membersRes, subsRes, pastRes, attRes] = await Promise.all([
  ...
  const pastIds = new Set(((pastRes.data ?? []) as { id: string }[]).map((s) => s.id));
  ...
  for (const a of att) {
    ...
    if (!lastCame.has(a.member_id) || d > (lastCame.get(a.member_id) as string)) lastCame.set(a.member_id, d);
    if (pastIds.has(a.session_id)) recent.add(a.member_id);
    if (a.resolution === "pay_later") owes.set(a.member_id, (owes.get(a.member_id) ?? 0) + 1);
  }
    ...
    <>
      ...
      {params.error ? (
        <div className="note warn" role="alert" style={{ marginBottom: 12 }}>
          {params.error.includes("members_phone_key") ? "Someone with that number is already a member." : params.error}
        </div>
      ) : null}
      ...
      <div className="stack">
        <details className="details">
          ...
          <form action={addMember}>
            ...
            <div className="field">
              ...
              <input id="phone" name="phone" type="tel" inputMode="tel" placeholder="07… or +94…" />
            </div>
        ...
        {shown.length === 0 ? (
          <div className="card empty">
            {members.length === 0 ? "No members yet. Add the first one above." : "No one matches."}
          </div>
        ) : (
          <ul className="list">
            {shown.map((m) => {
              ...
              if (sub) {
                ...
                if (renewed.has(m.id)) planText += `, renewed for ${monthName(nextMonth)}`;
              }
              return (
                <li key={m.id}>
                  <div className="row-main">
                    ...
                    <Link href={`/members/${m.id}`} className="grow row-link">
                      ...
                      <div className="sub">{last ? `Last came ${formatDate(last)}` : "No visits in the last 90 days"}</div>
                      {owed > 0 ? (
                    ...
                    {wa ? (
                      <a href={wa} target="_blank" rel="noopener noreferrer" className="btn btn-outline btn-small">
